...
int rm(uint currentcluster, char* file_name) {
	...
	DIR = getdir(currentcluster, fname);
	printf("TESTING DIR_entry name: %s and %d\n", DIR.DIR_Name, currentcluster);
...
int ls(uint currentcluster) {
	...
	while(1) {
		...
		while(offset < getsectoroffset(getfirstsector(currentcluster))+getsectorbytes()) {
			//temp = getsectoroffset(getfirstsector(currentcluster))+getsectorbytes();
...
struct DIR getdir(uint cluster, char* dirname) {
	...
	long temp;
	printf("CLUSTER being passed in is %d\n", cluster);
	if(cluster == 0) {
		cluster = 2;
	}
	...
	while(1) {
		...
		printf("OFFSET IS %d\n", offset);
		fseek(file, offset, SEEK_SET);
		temp = getsectoroffset(getfirstsector(cluster))+getsectorbytes();
		while(offset < getsectoroffset(getfirstsector(cluster))+getsectorbytes()) {
			...
			offset += 32;
			printf("OFFSET PLUS 32: %d\n", offset);
		}

		cluster = getfat(cluster);
		printf("CLUSTER ISSS %d\n", cluster);
